...
import { ArrowRight, Clock, CircleCheck as CheckCircle2, Zap, Award } from "lucide-react";
...
export const CourseCard: React.FC<CourseCardProps> = ({ course, onOpenModal }) => {
  ...
  return (
    <div
      className="group perspective-1000 w-full min-h-[480px] cursor-pointer"
    ...
    >
      ...
      >
        {/* Front Side */}
        <div className="absolute inset-0 w-full h-full backface-hidden rounded-[2.5rem] p-6 sm:p-8 lg:p-10 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl flex flex-col justify-between overflow-y-auto group-hover:shadow-2xl group-hover:shadow-blue-500/10 transition-all">
          ...
          <div className="flex-grow min-h-0">
            <h3 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white mb-3 sm:mb-4 tracking-tight leading-tight">
              {course.title}
            </h3>
            <p className="text-slate-600 dark:text-slate-400 text-sm sm:text-base lg:text-lg leading-relaxed break-words">
              {course.description}
            </p>
          </div>

          <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between flex-shrink-0">
            <div>
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Starting at</span>
              <span className="text-xl sm:text-2xl lg:text-3xl font-black text-slate-900 dark:text-white">{course.price}</span>
            </div>
        ...
        {/* Back Side */}
        <div
          className="absolute inset-0 w-full h-full backface-hidden rounded-[2.5rem] p-6 sm:p-8 lg:p-10 bg-slate-900 dark:bg-slate-950 border border-slate-800 shadow-2xl flex flex-col justify-between overflow-y-auto"
        ...
        >
            ...
            <div className="space-y-3 mb-8">
              {course.features.slice(0, 4).map((skill, i) => (
                <div key={i} className="flex items-start gap-3 text-slate-300">
                  ...
                  <span className="font-bold text-sm lg:text-base break-words">{skill}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-3 flex-shrink-0">
            <button
              ...
              className="w-full bg-emerald-600/10 text-emerald-500 border border-emerald-500/20 py-3 sm:py-4 rounded-2xl font-black text-base sm:text-lg hover:bg-emerald-600 hover:text-white transition-all flex items-center justify-center gap-3 group/btn"
            >
            ...
            <Link
              ...
              className="w-full bg-blue-600 text-white py-3 sm:py-4 rounded-2xl font-black text-base sm:text-lg hover:bg-blue-700 transition-all shadow-2xl shadow-blue-600/30 flex items-center justify-center gap-3 group/btn"
            >
